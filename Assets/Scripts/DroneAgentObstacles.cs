...
public class DroneAgentObstacles : Agent
{
    [Header("References")]
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        // 1. Reward per avvicinarsi al goal 
        float delta = previousDistance - currentDistance;
        AddReward(delta * 0.5f);
        //AddReward(Mathf.Clamp(delta * 0.5f, -0.05f, float.MaxValue));
        ...
        // Non termina l'episodio: lascia che il drone si corregga
        ...
        if (heightError < -20f) // più permissivo, era -10f
            AddReward(-0.003f); // era 0.005
        ...
        // PROBLEMA: con ostacoli il drone potrebbe fare traiettorie più lunghe
        //if (Vector3.Distance(transform.position, spawnPoint.position) > maxDistance  || transform.position.y < 0f))

        // FIX: uso la distanza assoluta dal goal, non dallo spawn
        if (currentDistance > maxDistance || transform.position.y < 0f)
        {
            AddReward(-5f);
            EndEpisode();
        }
    ...
    */
    public override void Heuristic(in ActionBuffers actionsOut)
    {
        ...
        c[3] = Input.GetKey(KeyCode.Q) ? -1f : Input.GetKey(KeyCode.E) ?  1f : 0f; // yaw
    }
